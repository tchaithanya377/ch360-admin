...
const Faculty = () => {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        ...
        const response = await facultyApiService.getFaculty(params);
        const fetchedFaculty = response.results.map((member) => ({
          id: member.id,
          name: `${member.first_name} ${member.last_name}`.trim(),
          empID: member.employee_id || '',
          designation: member.designation || '',
          depRole: member.department || '',
  ...
  const saveChanges = async () => {
    try {
      await facultyApiService.updateFaculty(selectedFaculty.id, selectedFaculty);
