...
export default function Waivers() {
  ...
  const [filters, setFilters] = useState({ waiver_type: '', is_active: '', approved_by: '', search: '', ordering: '-created_at', page: 1 });

  const fetchData = async () => {
    setLoading(true); setError(null);
    try {
      const res = await FeesAPI.listWaivers(filters);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-end">
        <div>
          ...
          <input className="border rounded px-2 py-1" value={filters.waiver_type}
                 onChange={e => setFilters(f => ({ ...f, waiver_type: e.target.value, page: 1 }))} />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm text-gray-600">Approved By (ID)</label>
          <input className="border rounded px-2 py-1" value={filters.approved_by}
                 onChange={e => setFilters(f => ({ ...f, approved_by: e.target.value, page: 1 }))} />
        </div>
        <div>
            ...
            <tbody>
              {data.map(row => (
                <tr key={row.id}>
                  ...
                  <td className="p-2 border">{row.waiver_type}</td>
                  ...
                  <td className="p-2 border">{row.approved_by_username}</td>
                  <td className="p-2 border">
                    {!row.is_active && (
                      <button className="px-2 py-1 text-xs bg-green-600 text-white rounded"
                              onClick={async () => { await FeesAPI.approveWaiver(row.id); fetchData(); }}>
                        Approve
                      </button>
                    )}
                  </td>
